...
function initializeNavigation() {
    ...
    if (hamburger && navMenu) {
        ...
        // Ensure Tickets, Vote, Admin and Login buttons are present in navMenu (for mobile)
        // Check if Tickets link exists, if not create it
        let ticketsLink = navMenu.querySelector('a[href="ticket.html"]');
        if (!ticketsLink) {
            ticketsLink = document.createElement('a');
            ticketsLink.href = 'ticket.html';
            ticketsLink.className = 'nav-link';
            ticketsLink.textContent = 'Tickets';
            // Insert before the Contact link or at the end if Contact doesn't exist
            const contactLink = navMenu.querySelector('a[href="contact.html"]');
            if (contactLink) {
                navMenu.insertBefore(ticketsLink, contactLink);
            } else {
                const authLinks = navMenu.querySelector('.auth-links');
                if (authLinks) {
                    navMenu.insertBefore(ticketsLink, authLinks);
                } else {
                    navMenu.appendChild(ticketsLink);
                }
            }
        }

        // Check if Vote link exists, if not create it
        let voteLink = navMenu.querySelector('a[href="vote.html"]');
        if (!voteLink) {
            voteLink = document.createElement('a');
            voteLink.href = 'vote.html';
            voteLink.className = 'nav-link';
            voteLink.textContent = 'Vote';
            // Insert before the Contact link or at the end if Contact doesn't exist
            const contactLink = navMenu.querySelector('a[href="contact.html"]');
            if (contactLink) {
                navMenu.insertBefore(voteLink, contactLink);
            } else {
                const authLinks = navMenu.querySelector('.auth-links');
                if (authLinks) {
                    navMenu.insertBefore(voteLink, authLinks);
                } else {
                    navMenu.appendChild(voteLink);
                }
            }
        }

        // Ensure Admin button is present in navMenu (for mobile)
